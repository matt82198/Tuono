...
local function luaFilesOnDisk()
  ...
  if package.config:sub(1, 1) == "\\" then
    cmd = 'dir /b /s "' .. harness.root:gsub("/", "\\") .. '\\*.lua" 2>nul'
  else
    cmd = 'find "' .. harness.root .. '" -name "*.lua" -type f 2>/dev/null'
  end
  local pipe = io.popen(cmd)
  if not pipe then return out end
  for line in pipe:lines() do
    local norm = line:gsub("\\", "/")
    local rel = norm:sub(#harness.root + 2)
...
describe("toc", function()
  local tocPath = harness.root .. "/Tuono.toc"
  ...
  it("every listed file exists on disk", function()
    ...
    for _, rel in ipairs(harness.tocFiles()) do
      if not fileExists(harness.root .. "/" .. rel) then
