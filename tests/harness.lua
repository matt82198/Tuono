...
-- THE ADDON LIVES IN A SUBDIRECTORY. The repo root holds docs/, tests/, tools/ and the
-- shipping addon under Tuono/. An earlier version of this harness assumed the addon was
-- at the root, which is how the deployed AddOns folder looks but not how the repo does.
harness.addonDir = harness.root .. "/Tuono"

function harness.tocFiles()
  local path = harness.addonDir .. "/Tuono.toc"
...
function harness.load(opts)
  ...
  for _, rel in ipairs(harness.tocFiles()) do
    if not (opts.skip and opts.skip[rel]) then
      local path = harness.addonDir .. "/" .. rel
