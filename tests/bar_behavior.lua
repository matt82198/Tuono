...
-- Solution: assertions over Tuono.Engine.Evaluate() output (the actual bar contents).

local function runBarBehaviorTests(Tuono, stub, assert_eq, assert_true, assert_false, test)
  ...
  test("bar behavior: sequence advances (finisher at 5 CP, followed by builders)", function()
    ...
    for i, e in ipairs(r.queue) do
      if e.spellID == Tuono.SpellIDs.dispatch then
        finisherFound = true
        finisherPos = i
        break
      end
  ...
  test("bar behavior: all queue entries are castable (no unknown cooldowns)", function()
    ...
    if r.queue then
      for i, entry in ipairs(r.queue) do
        ...
        local cooldown = Tuono.State.cooldowns[spellID]
  ...
  test("bar behavior: levelling build (2-3 known abilities) queues only known spells", function()
    ...
    for k in pairs(Tuono.State.cooldowns) do
    ...
    for i, entry in ipairs(r.queue) do
      local spellID = entry.spellID
      assert_true(Tuono.State.knownSpells[spellID] == true,
  ...
  test("bar behavior: stable evaluation (same state = same queue; changed state = different queue)", function()
    ...
    if Tuono.State.RefreshFast then Tuono.State.RefreshFast() end
    Tuono.State.comboPoints = Tuono.State.comboPointsMax
    Tuono.State.cooldowns.dispatch = { known = true, ready = true, remaining = 0 }
    local r3 = evalBar()
    local seq3 = {}
    if r3.queue then
    ...
    assert_eq(seq3[1], Tuono.SpellIDs.dispatch,
