...
test("aoeDetected path - detects blade flurry in queue", function()
  ...
  -- SUPERSEDED, DELIBERATELY. aoeDetected used to key off MEMBERSHIP of
  -- GetRotationSpells -- a CAPABILITY SET, as AssistReader's own comment says. Blade
  -- Flurry is permanently in an Outlaw's rotation table, so the flag was a constant
  -- true and pinned the addon to the AoE list on every fight. It now keys off the LIVE
  -- pick. Membership is still tracked, as cleaveCapable, which is what this asserts.
  -- New intent covered by tests/test_mode.lua.
  assert_true(Tuono.Assist.cleaveCapable, "spec CAN cleave: Blade Flurry is in the rotation table")
  assert_false(Tuono.Assist.aoeDetected, "but capability is not a live target count")
...
test("aura-infra: all-secret auras without cast → degraded=true", function()
  ...
  -- FIELD RENAMED, INTENT UNCHANGED. An added aura that correlates to no cast of ours
  -- used to set buffs.degraded -- which fires for every food, flask, raid buff and
  -- trinket proc the player gains, making the flag a function of how buffed they are.
  -- It is now recorded as buffs.deltaBlind, and `degraded` is reserved for a genuine
  -- inability to read. New intent covered by tests/test_statetracker.lua.
  --
  -- The property guarded here is unchanged: an unidentifiable secret aura must be
  -- NOTICED AND RECORDED rather than throwing.
  assert_true(Tuono.State.buffs.deltaBlind, "uncorrelated secret aura recorded as a blind delta")
  assert_true(true, "no error thrown with a secret aura")
end)
...
test("live-queue: aoeDetected still detects blade flurry from rotationSet", function()
  ...
  -- SUPERSEDED: see the note on the aoeDetected test above. Membership now reports as
  -- cleaveCapable; aoeDetected means "Blizzard is recommending the cleave right now".
  assert_true(Tuono.Assist.cleaveCapable, "Blade Flurry membership still tracked")
